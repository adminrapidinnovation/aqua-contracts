...
interface IStrategy {
    ...
    function setMinTimeToWithdraw(uint _newMinTime) external;
...
contract TimelockController is AccessControl {
    ...
    function setDevWalletAddress(address payable _devWalletAddress) public onlyRole(EXECUTOR_ROLE){
    ...
    function setSettings(
        ...
        IStrategy(_stratAddress).setSettings(
        _entranceFeeFactor,
        _withdrawFeeFactor,
        _controllerFee,
        _buyBackRate,
    ...
    function setGov(address _stratAddress,address _govAddress) public virtual onlyRole(EXECUTOR_ROLE) {
        IStrategy(_stratAddress).setGov(_govAddress);
    }

    function setOnlyGov(address _stratAddress,bool _onlyGov) public virtual onlyRole(EXECUTOR_ROLE) {
        IStrategy(_stratAddress).setOnlyGov(_onlyGov);
    }

    function setUniRouterAddress(address _stratAddress,address _uniRouterAddress)
        public
        virtual
        onlyRole(EXECUTOR_ROLE)
    {
        IStrategy(_stratAddress).setUniRouterAddress(_uniRouterAddress);
    }

    function setBuyBackAddress(address _stratAddress,address _buyBackAddress)
        public
        virtual
        onlyRole(EXECUTOR_ROLE)
    {
        IStrategy(_stratAddress).setBuyBackAddress(_buyBackAddress);
    }

    function setRewardsAddress(address _stratAddress,address _rewardsAddress)
        public
        virtual
        onlyRole(EXECUTOR_ROLE)
    {
        IStrategy(_stratAddress).setRewardsAddress(_rewardsAddress);
    }
    ...
    function inCaseTokensGetStuck(
        ...
        IStrategy(_stratAddress).inCaseTokensGetStuck(_token,_amount,_to);
    }

     function setMinTimeToWithdraw(address _stratAddress,uint256 newMinTimeToWithdraw)
        public virtual onlyRole(EXECUTOR_ROLE) {
            IStrategy(_stratAddress).setMinTimeToWithdraw(newMinTimeToWithdraw);
        }
    // // In case new vaults require functions without a timelock as well, hoping to avoid having multiple timelock contracts
